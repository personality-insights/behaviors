...
describe('i18n (es)', () => {
  ...
  it('get description for scenario', () => {
    ...
    assert.equal(scenarioData.name, 'Alimentos Saludables', 'Got expected scenario data');
  });

  it('get description for industry', () => {
    const data = new I18nData('es');
    const industryData = data.type('industries').id('industry.media');
    assert.equal(industryData.name, 'Medios', 'Got expected industry data');
  });

  it('get description for category', () => {
    const data = new I18nData('es');
    const categoryData = data.type('categories').id('category.risk');
    assert.equal(categoryData.name, 'Riesgo', 'Got expected category data');
  });
